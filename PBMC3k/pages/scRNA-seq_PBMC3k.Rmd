...
table(pbmc$groups, pbmc$annotation)
```
...
pbmc@meta.data %>% 
  ...
  ggplot(aes(annotation, fill=groups)) + geom_bar()
...
```{r}
pbmc@meta.data %>% 
  dplyr::count(annotation,groups) %>%
  dplyr::group_by(annotation) %>%
  dplyr::mutate(prop=prop.table(n)) %>%
  #head()
  ggplot(aes(annotation, prop, fill=groups)) + geom_bar(stat='identity')
```
...
pbmc.small[[]] %>% group_by(selected_cells) %>% count() %>% ungroup()

pbmc.small@meta.data
```
...
```{r}
DimPlot(pbmc, split.by = 'annotation', ncol=3)
```
...
```{r}
bulk <- AggregateExpression(pbmc, group.by = "annotation", return.seurat = TRUE)
Cells(bulk)
```
