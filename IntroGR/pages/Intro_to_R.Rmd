...
<center>![](../pics/Why_learn_R.webp){width="400"} [Source](https://aarababdellah.medium.com/what-is-r-programming-language-used-for-f600895e0416)</center>
...
2.  **Use the R Studio already installed on the machines in the room.**
...
You should see your cursor on the left-hand side of the screen blinking.
That window represents the R console.
It can be used to “talk” with R through commands that it can understand!
...
The analyses we are going to perform require specific *functions* that are not included in the basic set of functions in R.
These functions are collected in specific *packages*.
R packages are extensions to the R programming language that contain code, data and documentation which help us performing standardized workflows.
...
> 🤔 Exercise: Identify cars with 5 gears and horse power \> 100.
...
mtcars %>% 
  ...
  pivot_longer(cols=!cars, names_to = "keys", values_to = "value") %>%
...
mtcars %>% slice(1:4) %>% select(1:4) %>% 
  ...
  #reshape2::melt(id.vars="cars", measure.vars= c("mpg", "cyl", "disp", "hp"),variable.name="keys",     value.name="value") %>%
...
#   pull(dir) %>% quantile()
# 
#   #Rename the last column (new = old)
...
ggplot is a powerful and flexible library of tidyverse to create plots.

<center>

![](../pics/plots.png){width="400"}

[*https://r-graph-gallery.com/*](https://r-graph-gallery.com/){.uri}
...
Can you modify the code to annotate each point with its car label?
...
> 🤔 More exercises:
>
> -   Make all data points size 4 and blue, with red stroke of size 2.
>
> -   Size according to the number of gears

```{r, eval=TRUE, echo=FALSE, fig.width=6}
mtcars %>% ggplot(aes(mpg, wt )) + 
  geom_point(size=4, fill='blue', colour='red', shape=21, stroke=2)
...
Here, we use additional Aesthetics to change the appearance of the axes and modify the colors of the separate density plots.
